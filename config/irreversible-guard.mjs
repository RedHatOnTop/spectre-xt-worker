#!/usr/bin/env node
// irreversible-guard — the one thing that must survive `defaultMode: "dontAsk"`.
//
// With no approval prompts, the only barrier between an injected instruction and an
// unrecoverable action is a hook. `permissions.deny` is real — the harness enforces it — but
// it is prefix-matched and scoped to a single tool name, so `Bash(rm -rf:*)` misses
// `sh -c "rm -rf x"`, misses `cd x && rm -rf .`, and misses the PowerShell tool entirely.
// Measured against a live session, not assumed.
//
...
//   2. actions that would disable this guard: writing settings.json, the hooks directory,
//      opencode's config, or re-entering an agent with its guard switched off.
//
...
//
// PreToolUse contract: exit 2 blocks the call and returns stderr to the model. Exit 0 allows.
// A crash must allow — a broken guard must never wedge every session.
...
const OVERRIDE_FILE = join(CLAUDE_DIR, '.allow-irreversible');
...
function block(reason, detail, overridable = true) {
  const escape = overridable
    ? `If the user genuinely wants it, they run it themselves, or they authorise a window by hand:\n\n` +
      `    New-Item -ItemType File -Force "${OVERRIDE_FILE}"\n\n` +
...
/** `cat ~/.claude/hooks/x; rm /tmp/a` is two statements, and neither writes the config. */
...
const REGENERABLE = /(node_modules|target|dist|build|\.next|\.turbo|__pycache__|\.pytest_cache|coverage|\.venv)\b/;
...
// Box profile (SPECTRE_WORKER_PROFILE=box): the Spectre worker. A
// disposable, self-contained appliance — OS reinstallable from USB,
// data mirrored by warp and git. Git policy on the box is explicit:
// commits, pushes, PR creation/review, branch creation are normal work;
// merges, deletions, and anything that erases history are human-only.
...
// Both agents' configuration is protected from both agents.
const PROTECTED_SHAPE =
  /\.claude[\\/](settings|hooks)|\.claude\.json|\.allow-irreversible|\.config[\\/]opencode[\\/](opencode\.json|plugin)/;
const WRITE_VERB =
...
function isProtected(p) {
  if (!p) return false;
  const abs = norm(p);
  const cd = norm(CLAUDE_DIR);
  const oc = norm(join(HOME, '.config', 'opencode'));
  return (
    abs === `${cd}/settings.json` ||
    abs === `${cd}/settings.local.json` ||
...
if (['Write', 'Edit', 'NotebookEdit', 'MultiEdit'].includes(tool)) {
  if (isProtected(input.file_path ?? input.notebook_path)) {
    block('This file decides what the guard blocks.', `${tool} -> ${input.file_path ?? input.notebook_path}`, false);
  }
  allow();
}

if (!['Bash', 'PowerShell'].includes(tool)) allow();
...
// write. `cat ~/.claude/hooks/x; rm /tmp/a` used to trip this.
