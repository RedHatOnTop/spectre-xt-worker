...
if [[ -f "${slack_env}" ]]; then
  ...
  if RUN_AS_USER bash -lc 'command -v claude' >/dev/null 2>&1; then ok "claude present (bridge executor)"; else warn "claude not on PATH — bridge runs will fail (spawn_enoent)"; fi
