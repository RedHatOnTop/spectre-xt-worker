...
def ingest_workers(
    ...
    *,
    ...
    for name, entry in workers.items():
        ...
        _ingest_transport(store, name, entry, now)
...
def _ingest_transport(store: Store, name: str, entry: dict, now: float) -> None:
    tmux = entry.get("tmux")
    if not tmux:
        return
    reachable = _tmux_alive(str(tmux))
    if reachable is None:
        return
    kind = "transport.reachable" if reachable else "transport.down"
    if _LAST_TRANSPORT.get(name) == kind:
        return
    _LAST_TRANSPORT[name] = kind
    store.ingest(
        {
