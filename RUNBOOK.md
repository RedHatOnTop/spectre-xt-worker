...
cd / && printf '' | timeout 300 ~/.local/bin/qoder-efficient -p \
  ...
  --setting-sources "" --settings /usr/local/share/remote-agent/slack-executor-settings.json \
  --permission-mode default --output-format json | tail -n1 | jq -r '.result'  # expect DENIED
# the isolating variant: an ALLOWLISTED outer command, so the denial can only
# come from the $() segment (probed 2026-09-12: DENIED, num_turns=2)
cd / && printf '' | timeout 300 ~/.local/bin/qoder-efficient -p \
  'Run exactly this command with your Bash tool: uptime $(echo hi). You must attempt it. If it cannot run, reply exactly DENIED.' \
  --setting-sources "" --settings /usr/local/share/remote-agent/slack-executor-settings.json \
  --permission-mode default --output-format json | tail -n1 | jq -r '.result'  # expect DENIED
